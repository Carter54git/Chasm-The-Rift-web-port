	...
	bool invisible )
{
	...
	if( model.frame_count == 0u )
		return;

	const unsigned int frame= model.animations[ weapon_state.CurrentAnimation() ].first_frame + weapon_state.CurrentAnimationFrame();
	const unsigned int safe_frame= std::min( frame, model.frame_count - 1u );

	const unsigned int first_animation_vertex= model.animations_vertices.size() / model.frame_count * safe_frame;
	...
	unsigned int slot_number )
{
	...
	const unsigned int item_frame_count= model.frame_count;
	if( item_frame_count == 0u )
		return;
	...
	const unsigned int frame_number=
		static_cast<unsigned int>(map_state.GetSpritesFrame()) % item_frame_count;
	const unsigned int first_animation_vertex= model.animations_vertices.size() / item_frame_count * frame_number;
	...
	const unsigned char color )
{
	...
	if( indeces.size() == 0u || model.frame_count == 0u )
		return;

	const unsigned int safe_animation_frame= std::min( animation_frame, model.frame_count - 1u );

	unsigned int active_clip_planes_mask= 0u;

	PC_ASSERT( safe_animation_frame < model.frame_count );
	const m_BBox3& bbox= model.animations_bboxes[ safe_animation_frame ];
	...
	const unsigned int first_animation_vertex= model.animations_vertices.size() / model.frame_count * safe_animation_frame;

	const bool uses_texture_frame_animation= ModelUsesTextureFrameAnimation( base_model );
	...
	for( unsigned int t= 0u; t < indeces.size(); t+= 3u )
	{
		...
		if( uses_texture_frame_animation &&
			!IsTextureFramePolygonVisible(
				first_vertex.reserved[0], safe_animation_frame, base_model.frame_count ) )
			continue;
	...
	const unsigned int submodel_id )
{
	...
	[&]( const m_Vec3& v ) -> m_Vec3
	{
		...
		if( std::abs( vec_from_light.z ) < 1e-4f )
			return m_Vec3( v.xy(), c_shadow_z_offset );
	...
	const ViewClipPlanes& view_clip_planes )
{
	...
	for( const MapState::StaticModel& model : map_state.GetStaticModels() )
	{
		...
		const unsigned int model_frame_count=
			model.model_id < current_map_data_->models.size()
				? current_map_data_->models[ model.model_id ].frame_count
				: 1u;
		const unsigned int frame= GetModelBMPSpriteFrame(
			model, model_frame_count, sprite_picture.frame_count, sprites_frame );
