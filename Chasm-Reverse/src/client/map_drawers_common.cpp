...
unsigned int GetModelBMPSpriteFrame(
	const MapState::StaticModel& model,
	unsigned int model_frame_count,
	unsigned int sprite_frame_count,
	float sprites_frame )
{
	if( sprite_frame_count == 0u )
		return 0u;

	if( !model.animation_playing )
		return model.animation_frame % sprite_frame_count;

	if( model_frame_count > 1u )
		return model.animation_frame % sprite_frame_count;

	const unsigned int phase= GetModelBMPSpritePhase( model );
	return static_cast<unsigned int>( sprites_frame + phase ) % sprite_frame_count;
}

bool ModelUsesTextureFrameAnimation( const Model& model )
{
	if( model.frame_count <= 1u )
		return false;

	const unsigned int verts_per_frame= model.animations_vertices.size() / model.frame_count;
	if( verts_per_frame == 0u )
		return false;

	const char* const frame0= reinterpret_cast<const char*>( model.animations_vertices.data() );
	const char* const frame1= frame0 + verts_per_frame * sizeof( Submodel::AnimationVertex );
	return std::memcmp(
		frame0, frame1,
		verts_per_frame * sizeof( Submodel::AnimationVertex ) ) == 0;
}

bool IsTextureFramePolygonVisible(
	const unsigned char polygon_flags,
	const unsigned int animation_frame,
	const unsigned int frame_count )
{
	const unsigned char state_bits= polygon_flags & 0x30u;
	if( state_bits == 0u )
		return true;
	...
	const unsigned int midpoint= ( frame_count + 1u ) / 2u;

	if( ( state_bits & 0x10u ) != 0u )
		return animation_frame < midpoint;

	if( ( state_bits & 0x20u ) != 0u )
		return animation_frame >= midpoint;
...
unsigned int ClampStaticModelAnimationFrame(
	const MapData& map_data,
	const unsigned int model_id,
	const unsigned int frame )
{
	if( model_id >= map_data.models.size() )
		return 0u;

	const unsigned int frame_count= map_data.models[ model_id ].frame_count;
	if( frame_count == 0u )
		return 0u;

	return std::min( frame, frame_count - 1u );
}
