...
bool IsSwitchModel( const MapData& map_data, const unsigned int model_id )
{
	return
		model_id < map_data.models_description.size() &&
		static_cast<ACode>( map_data.models_description[ model_id ].ac ) == ACode::Switch;
}
	...
	, last_tick_time_(map_start_time)
{
	...
	for( unsigned int m= 0u; m < static_models_.size(); m++ )
	{
		...
		out_model.animation_playing= false;
	}
...
void MapState::Tick( const Time current_time )
{
	...
	for( StaticModel& model : static_models_ )
	{
		...
		if( model.animation_playing && model_geometry.frame_count > 1u )
		{
			model.animation_frame=
				static_cast<unsigned int>( GameConstants::animations_frames_per_second * time_since_map_start_s ) %
				( model_geometry.frame_count - 1u );
		}

		model.animation_frame= ClampStaticModelAnimationFrame(
			*map_data_, model.model_id, model.animation_frame );
	}
...
void MapState::ProcessMessage( const Messages::StaticModelState& message )
{
	...
	const unsigned int previous_model_id= static_model.model_id;
	...
	static_model.animation_playing= message.animation_playing;

	unsigned int network_frame= 0u;
	if( static_model.model_id < map_data_->models.size() )
	{
		network_frame= ClampStaticModelAnimationFrame(
			*map_data_, static_model.model_id, message.animation_frame );
	}

	if( previous_model_id != message.model_id ||
		IsSwitchModel( *map_data_, static_model.model_id ) ||
		!message.animation_playing )
		static_model.animation_frame= network_frame;
}
