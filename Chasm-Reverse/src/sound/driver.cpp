...
static const SDL_AudioDeviceID g_first_valid_device_id=
...
Driver::Driver()
{
	...
	// ~ 1 callback call per two frames (60fps); web needs a larger buffer to avoid crackle
#ifdef __EMSCRIPTEN__
	requested_format.samples= 4096;
#else
	...
	if( device_count <= 0 )
	{
			...
			obtained_format.format   == requested_format.format )
		{
			...
			Log::Info( "Open audio device: (default web)" );
		}
	}
	else
#endif
	...
	{
#ifdef __EMSCRIPTEN__
		Log::Warning( "Can not open any audio device (web)" );
#else
