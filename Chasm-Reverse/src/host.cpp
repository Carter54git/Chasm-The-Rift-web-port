	...
	, commands_processor_( settings_ )
{
	...
	settings_.SetSetting( SettingsKeys::software_rendering, true );
	settings_.SetSetting( "r_software_use_gl_screen_update", false );
	settings_.SetSetting( SettingsKeys::fullscreen, false );
	settings_.SetSetting( SettingsKeys::window_width, 1280 );
	settings_.SetSetting( SettingsKeys::window_height, 768 );
	settings_.SetSetting( SettingsKeys::mouse_sensetivity, 0.5f );
#endif
...
#else
		Log::FatalError( "OpenGL renderer is not supported in web build" );
#endif
	}
	else
	{
